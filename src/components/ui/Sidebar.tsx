...
export function Sidebar({ showUsers }: SidebarProps) {
	...
	return (
		<>
			<aside className="border-border-muted bg-sidebar fixed inset-y-0 left-0 z-40 hidden w-14 flex-col items-center border-r py-3 md:flex">
				<div className="bg-blue mb-4 flex h-8 w-8 items-center justify-center rounded-lg">
					<svg
						viewBox="0 0 24 24"
						fill="none"
						stroke="#0d1117"
			...
			<nav className="border-border-muted bg-sidebar fixed inset-x-0 bottom-0 z-40 flex h-14 items-center justify-around border-t md:hidden">
				{navItems.map((item) => (
					<Link
						key={item.href}
						href={item.href}
